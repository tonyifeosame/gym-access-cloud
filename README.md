...
| `EMAIL_PROVIDER` | unset | `brevo_api`, `smtp` or `log`. Unset means no outbound mail: a forgotten-password request writes the reset link to the log for an administrator to act on, exactly as before. `log` writes the whole message to the log (development only — it puts the link in the log) |
| `BREVO_API_KEY` | — | For `EMAIL_PROVIDER=brevo_api`: the transactional **API key** (`xkeysib-…`) from Brevo → SMTP & API → API keys — not the SMTP key. Sends over HTTPS (443) to `api.brevo.com`, which is the provider to use where outbound SMTP is blocked, as it is on Render. The sender in `EMAIL_FROM` must be a validated sender or authenticated domain in Brevo |
| `BREVO_API_URL` | `https://api.brevo.com/v3` | The API base. For tests; never set in a deployment |
